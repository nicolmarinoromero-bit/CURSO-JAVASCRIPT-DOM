...
function computeBudget(income, expenses) {
  ...
  const save10 = +(income * 0.1).toFixed(2);
  const save20 = +(income * 0.2).toFixed(2);
...
function setOutputs({ income, totalExpenses, balance, save10, save20 }) {
  ...
  if (balance < 0) {
    ...
    resultsArea.innerHTML = `<p class="success">💡 Ahorro sugerido: entre ${fmt.format(
      save10
    )} y ${fmt.format(save20)}.</p>`;
  }
}
...
form.addEventListener("submit", (e) => {
  ...
  if (Number.isNaN(rawIncome) || rawIncome < 0) {
    return showError(
      "❌ Ingresos inválidos. Introduce un número mayor o igual a 0."
    );
  }
  if (expenses.some((n) => n < 0)) {
    return showError(
      "❌ Algunos gastos son negativos. Verifica tus líneas de gasto."
    );
  }

  // Cálculos
  const { totalExpenses, balance, save10, save20 } = computeBudget(
    rawIncome,
    expenses
  );
